defmodule HelloSocketsWeb.WildcardChannelTest do
  ...
  describe "handle_in/3 ping" do
    test "a pong response is provided" do
      assert {:ok, _, socket} = socket(UserSocket, nil, %{}) |> subscribe_and_join("wild:2:4", %{})

      ref = push(socket, "ping", %{})
      reply = %{ping: "pong"}

      # We are using ^reply to pin a map value to the reply we receive. If we
      # didn’t use ^reply and just left the response inline in the arguments, then
      # values such as %{ping: "pong", extra: true} would still cause our tests to
      # pass.
